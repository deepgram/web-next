					...
					return (
						<li class={pathname.includes(nav.to) ? "active" : ""}>
							<div class="navlink-container flex items-center">
								...
								{nav.children && <SideNavList className={`${nav.children ? "border-l border-cloud" : ""} ${isExpandedFunc(nav) ? "" : "hidden"} ${isExpandedFunc(nav) ? "HELLO" : "GOODBYE"}`} navStructure={nav.children} pathname={pathname} />}
						</li>
					)
				})}
			</ul>
