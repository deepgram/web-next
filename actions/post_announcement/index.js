...
const { SHORT_URL_FUNCTION_URL, SLACK_WEBHOOK_URL } = process.env;
...
const processPost = async (filename) => {
	...
	for (const source of utm_sources) {
		const shortUrl = await getShortUrl(`${postUrl}?utm_source=${source.source}&utm_campaign=blog&utm_content=${slug}`);
		shortUrls.push({
			label: source.label,
			shortUrl,
		});
