""" Uses the real-time endpoint to calculate a transcript suitable for WER comparisons.
    Python version: 3.6+
    Dependencies (use `pip install X` to install a dependency):
      - websockets
    Usage:
      python concat-final.py -k 'YOUR_DEEPGRAM_API_KEY' /path/to/audio.wav
    Limitations:
        ...
        async def receiver(ws):
            ...
            async for msg in ws:
                ...
                if is_final and transcript != '':
                    if complete:
                        complete = f'{complete} {transcript}'
                    else:
                        complete = transcript
            print(f'{complete}')
