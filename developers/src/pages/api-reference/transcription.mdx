...
<div class="feature">
	...
	<main>
		...
		<ul>
			...
			<li class="notranslate feature-item">
				<strong>whisper:</strong> (Default)
				
				Deepgram's hosted version of OpenAI's Whisper model.
	...
	<main>
		<p>
			...
			<ul class="info">
				...
				<li>If the HTTP status code of the response to the callback POST request is unsuccessful (not 200-299),
				 Deepgram will retry the callback up to 10 times with a 30 second delay between attempts.</li>
			</ul>
		<p>
			To learn more, see
...
<div class="feature">
	...
	<main>
		...
		<p>By default, endpointing is enabled and finalizes a transcript after 10 ms of silence. You can customize the length of time used to detect whether a speaker has finished speaking by setting the <code>endpointing</code> parameter to an integer representing a millisecond value.</p>
		<p>Endpointing may be disabled by setting <code>endpointing=false</code>.</p>
		<p class="notranslate">
...
By default, the Deepgram streaming connection will time out with a NET-0001 error code if no audio is sent by the client for 10 seconds. (See [Error Handling](/api-reference/transcription/#error-handling) below for more information.)

To keep the websocket open without sending audio data, send the following JSON string:

`{ 'type': 'KeepAlive' }`

This will keep the streaming connection open for an additional 60 seconds. If no audio or additional KeepAlive messages are sent within the 60 second window, the streaming connection will close with a NET-0001. To avoid this error and keep the connection open, continue sending KeepAlive messages 3-5 seconds before the 60 second timeout window expires until you are ready to resume sending audio.
