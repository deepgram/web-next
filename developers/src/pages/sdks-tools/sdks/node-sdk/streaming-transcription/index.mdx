...
The `toggleNumerals` function toggles the `numerals` feature on and off. The function takes in a boolean value to determine whether or not to enable the feature.

```js
deepgramLive.toggleNumerals(true); // enable numerals
deepgramLive.toggleNumerals(false); // disable numerals
```
