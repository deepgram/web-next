...
In the `umsdeepgram.xml` configuration file, add a `service-endpoint` child element to the `service-endpoints` element. You must specify the on-prem deployment’s IP address and port number in the `service-uri` attribute. On-prem’s URI is `/1/listen/stream` and is only a web socket (`ws`), not a secure web socket (`wss`).
