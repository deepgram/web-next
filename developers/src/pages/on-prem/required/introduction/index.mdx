...
title: Introduction
description: Learn about on-premises deployments of Deepgram products using AutoML-trained (TM) models.
type: prereq
order: 1
seo:
  metaTitle: Introduction
  metaDescription: Learn about on-premises deployments of Deepgram products using AutoML-trained (TM) models.
  keywords: on-premises, on-prem, deployment, introduction, required components, required
---
...
AutoML (TM) is a collection of tools and services which are used to produce trained AI models for the purpose automatic speech recognition (ASR). From an on-prem perspective, there is a solution called AutoML (TM) which enables users to customize Deepgram's ASR models with their own data.

## Required Components

Deepgram provides a variety of components available for on-prem deployment. This guide describes how you create a deployment using Deepgram’s required components. If you are interested in learning about and deploying Deepgram’s optional components on premises, see [Optional Components](/on-prem/optional-components/introduction/).
