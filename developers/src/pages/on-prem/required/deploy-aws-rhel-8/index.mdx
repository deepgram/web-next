...
1. Download the latest Deepgram Engine image from Docker:
   ...
   ```bash
   podman pull docker.io/deepgram/onprem-engine:3.43.4
   ```

2. Download the latest Deepgram API image from Docker:

   ```bash
   podman pull docker.io/deepgram/onprem-api:1.83.0
   ```

3. Download the latest Deepgram License Proxy Server image from Docker:

   ```bash
   podman pull docker.io/deepgram/onprem-license-proxy:1.3.0
   ```
...
```bash
podman run --rm -d --name license-proxy --network=host \
  docker.io/deepgram/onprem-license-proxy:1.3.0 -v serve \
  --license-key "LICENSE_KEY" --host 0.0.0.0 --port 8443 --status-port 8089 \
  && podman container logs license-proxy
```
...
podman run --security-opt=no-new-privileges --cap-drop=ALL --security-opt \
  label=type:nvidia_container_t --hooks-dir=/usr/share/containers/oci/hooks.d/ \
  --rm -d --name engine --network=host \
  -v ./config:/config:z -v ./models:/models:z \
  docker.io/deepgram/onprem-engine:3.43.4 -v serve /config/engine.toml \
  && podman container logs engine 
```

4. Start API

```bash
podman run --rm -d --name api --network=host -v ./config:/config:Z \
  -v ./models:/models:Z docker.io/deepgram/onprem-api:1.83.0 \
  -v serve /config/api.toml \
  && podman container logs api
```
