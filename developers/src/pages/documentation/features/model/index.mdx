...
Each model belongs to a tier. For self-serve customers, Deepgram provides Enhanced and Base model tiers.
...
<Alert type="info">
The Nova model works differently than our enhanced and base tiers and can be called by passing in Nova as model option or as tier option.
...
Using Deepgram's fully hosted Whisper Cloud instead of running your own version of Whisper provides many benefits. Some of these benefits include:
...
<Alert type="info">
Deepgram owns and maintains these Whisper models they aren't hosted or run by Open AI.
</Alert>

**There are some important things to understand about using Deepgram's Whisper Cloud:**

- Deepgram's Whisper model does not support automatic language detection. If you would like to transcribe audio in a specific language, you can do so by setting the language parameter in the query string. To learn more about languages, see [Language](/documentation/features/language/).
...
<Alert type="warning">
There is a 10 minute time out for all Deepgram models. Transcriptions requests that run longer than 10 minutes will return a 504 error.
</Alert>
