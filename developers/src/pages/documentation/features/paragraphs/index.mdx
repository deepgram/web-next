...
Deepgram's Paragraphs feature splits audio into paragraphs to improve transcript readability. When Paragraphs is enabled, Punctuation is automatically enabled, and paragraphs are determined based on the transcript's punctuation. When the Diarization feature is enabled and multiple speakers are present, paragraphs breaks are influenced by speaker changes. When the Multichannel feature is enabled, paragraphs breaks are influenced by channel changes.
...
When Paragraphs is enabled, Punctuation will automatically be enabled as well.
